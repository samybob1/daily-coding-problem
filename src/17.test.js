...
const solve = require("./17");

test("the longest absolute path to a file to be 0", () => {
  const fs = "";
  expect(solve(fs)).toBe(0);
});

test("the longest absolute path to a file to be 20", () => {
  const fs = "dir\n\tsubdir1\n\tsubdir2\n\t\tfile.ext";
  expect(solve(fs)).toBe(20);
});

test("the longest absolute path to a file to be 32", () => {
  const fs =
    "dir\n\tsubdir1\n\t\tfile1.ext\n\t\tsubsubdir1\n\tsubdir2\n\t\tsubsubdir2\n\t\t\tfile2.ext";
  expect(solve(fs)).toBe(32);
});
